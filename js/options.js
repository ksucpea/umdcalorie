...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector("#weight-enter").addEventListener("click", () => {
        chrome.storage.local.get(["weight"], storage => {
            ...
            chrome.storage.local.set({ "weight": data }).then(drawGraph());
        });
    });
...
const start = async () => {
    ...
    chrome.storage.local.get(["foods", "nutrition"], resp => {
        ...
        x.innerHTML = '<div style="width: 60px; text-align: center;">Qty</div><div style="width: 200px">Food</div><div style="width: 100px">Calories</div><div style="width: 100px">Total fat</div><div style="width: 100px">Saturated fat</div><div style="width: 100px">Trans fat</div><div style="width: 100px">Cholesterol</div><div style="width: 100px">Sodium</div><div style="width: 100px">Carbs</div><div style="width: 100px">Sugar</div><div style="width: 100px">Protein</div><div style="width: 100px">Calcium</div><div style="width: 100px">Potassium</div><div style="width: 100px">Vitamin C</div><div style="width: 100px">Iron</div>';
        el.appendChild(x);
        ...
        data.forEach(item => {
            try {
                ...
                div.innerHTML = `<div style="width: 60px; text-align: center;">${quant}</div><div style="width: 200px;height:16px;overflow:hidden;">${item.food}</div><div style="width: 100px;">${quant * facts[item.food].calories}</div><div style="width: 100px;">${(quant * facts[item.food].totalfat).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].saturatedfat).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].transfat).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].cholesterol).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].sodium).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].carbs).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].sugar).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].protein).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].calcium).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].potassium).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].vitaminc).toFixed(1)}g</div><div style="width: 100px;">${(quant * facts[item.food].iron).toFixed(1)}g</div>`;
...
function drawGraph() {
    ...
    el2.textContent = "";
    chrome.storage.local.get(["weight"], resp => {
        if (resp["weight"] && resp["weight"].length > 0) {

            let high = 0;
            let low = 1000;
            for (let i = 0; i < resp["weight"].length; i++) {
                if (resp["weight"][i].lbs > high) high = resp["weight"][i].lbs;
                if (resp["weight"][i].lbs < low) low = resp["weight"][i].lbs;
            }
            low -= 4;
            high += 4;
